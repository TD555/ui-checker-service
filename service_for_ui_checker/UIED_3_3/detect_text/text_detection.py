...
def visualize_texts(org_img, texts, shown_resize_height=None, show=False, write_path=None):
    img = org_img.copy()
    for text in texts:
        text.visualize_element(img, line=1)
...
def merge_intersected_texts(texts, img, merge_texts):
    '''
    ...
    while changed:
        ...
        for text_a in texts:
            merged = False
            for text_b in temp_set:
                # print(type(text_a), type(text_b))
                if text_a.is_intersected(text_b, bias=2) or (abs(dict(text_a.__dict__)['location']['top'] - dict(text_b.__dict__)['location']['bottom']) <= (math.ceil(4100 / img.shape[0]) +  round(img.shape[0] / img.shape[1])) * merge_texts**2
                    and (abs(dict(text_a.__dict__)['location']['left'] - dict(text_b.__dict__)['location']['left']) <= math.ceil(10000 / img.shape[1]) * merge_texts**2 or abs(dict(text_a.__dict__)['location']['right'] - dict(text_b.__dict__)['location']['right']) <= math.ceil(1000 / img.shape[1]) * merge_texts**2)): 
                    text_b.merge_text(text_a)
                    merged = True
                    changed = True
                    break
...
async def text_detection(input_file, id, merge_texts, output_file='../data/output', show=False, method='paddle', paddle_model=None):
    '''
    ...
    if method == 'google':
        ...
        if paddle_model is None:
            paddle_model = PaddleOCR(use_angle_cls=True, lang="en", enable_mkldnn=True)
        print(datetime.now())
        result = paddle_model.ocr(input_file, cls=True)
        print(datetime.now())
        texts = text_cvt_orc_format_paddle(result)
        texts = merge_intersected_texts(texts, img, merge_texts)
        texts = text_sentences_recognition(texts)
    else:
        raise ValueError('Method has to be "google" or "paddle"')

    # print(texts)
    save_detection_json(pjoin(ocr_root, id+'.json'), texts, img.shape)

    # print("[Text Detection Completed] Input: %s Output: %s" % (input_file, pjoin(ocr_root, id+'.json')))
